<template>
    <div class="page-main ecs-list-page list-page">
        <!-- <zt-scroller> -->
            ...
            <div class="page-body affix-page" :class="{'affixed': paginationAffix}">
                ...
                <!-- <zt-status v-for="item in ECS_STATUS" :status="ECS_STATUS" :value="item.value" :key="item.value" ></zt-status> -->
                    <el-table :affix="50" @filter-change="filterHandler" v-loading="loading" :data="tableDataList"  style="width: 100%" row-class-name="data-list" header-row-class-name="data-list" border class="data-list">
                        <!-- 名称 -->
                            ...
                            <template slot-scope="scope">
                                <div class="id-column table-row-list">
                                    <div class="text-nowrap table-row-list-item hoverToShowContent">
                                        ...
                                        <router-link :disabled="dropdownActive(scope.row.instanceStatus, modifyConfigActivedStatus)" :to="{name:'app.ecs.inst.detail',params:{id:scope.row.id,flavorId:scope.row.flavorId,item:scope.row}}">{{ scope.row.id }}</router-link> <copy-text class="hoverToShow finger-cursor" :bindText="scope.row.id" position="right"></copy-text>
                                    </div>
                                    <div class="flex text-nowrap flex-align-center table-row-list-item hoverToShowContent">
                                        ...
                                        <i v-if="!dropdownActive(scope.row.instanceStatus, modifyConfigActivedStatus)" class="iconfont icon-edit_people ml10 box-border amendInfo finger-cursor" style="padding: 1px" @click="editinstname(scope.row)"></i>
                        ...
                        <!-- 服务器组 -->
                        <!-- <el-table-column prop="servers" :label="getLabel('servers')"  min-width="50" class-name="pd0"> -->
                        <el-table-column prop="servers" :label="getLabel('servers')"   min-width="70">
                            <template slot-scope="scope">
                                {{$t('common.defaultServers')}}
                            </template>
                        </el-table-column>
                        <!-- 所在可用区域 -->
                        <el-table-column prop="region" :label="getLabel('region')" min-width="90" >
                            <template slot-scope="scope">
                                <div v-for="(item, key) in $options.filters.zone(scope.row.zone, 'array')" :key="key"> {{item}}</div>
                            </template>
                        </el-table-column>
                        <!-- IP地址 -->
                        <el-table-column prop="ipaddr" :label="getLabel('ipaddr')"  min-width="125">
                            <template slot-scope="scope">
                                <ul>
                                    <li v-if="!!scope.row.floatIp" class="text-nowrap">
                                        <span>{{ scope.row.floatIp }}</span>
                                        <span v-if="scope.row.floatIp.indexOf('10.3') != -1" class="color-secondary">({{$t('ecs.inst.list.linkIP')}})</span>
                                        <span v-if="scope.row.floatIp.indexOf('43.') != -1" class="color-secondary">({{$t('ecs.inst.list.publicNet')}})</span>
                                    </li>
                                    <li v-if="!!scope.row.ip" class="text-nowrap">
                                        <span>{{scope.row.ip}}</span>
                                        <span class="color-secondary">({{$t('ecs.inst.list.innerNet')}})</span>
                                    </li>
                                </ul>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column  :filter-multiple="false" prop="status" :label="getLabel('status')"  :filters="getDropdown('status')" :filtered-value="[status]"  min-width="80">
                            <template slot-scope="scope">
                                <el-tooltip class="item" effect="light" :disabled="scope.row.instanceStatus !== 'BUILD'" :content="scope.row.instanceStatus === 'BUILD' ? '预计需要3-6分钟' : ''" placement="top">
                                    <zt-status :status="ECS_STATUS" :value="scope.row.instanceStatus" class="text-nowrap status-column"></zt-status>
                                </el-tooltip>
                            </template>
                        </el-table-column>
                        <!-- 标签 -->
                        <!-- <el-table-column prop="servers" :label="getLabel('servers')" width="45">
                            <template slot-scope="scope">
                                <el-tooltip transition="scale-in" placement="top" effect="light">
                                    <div slot="content" class="font12">
                                        <div v-if="!!scope.row.labels.length">
                                            ...
                                            <el-row>
                                                <li>
                                                    <a @click="editLabel" class="blue font12">{{ $t('ecs.inst.list.dropdownBtns.editLable') }}</a>
                                                </li>
                                            </el-row>
                                        </div>
                                        <div v-else>
                                    ...
                                    <i class="iconfont icon-biaoqianmen-tianchong font16"></i>
                                </el-tooltip>
                            </template>
                        </el-table-column> -->
                        <!-- 操作系统 -->
                        <el-table-column prop="os" :label="getLabel('os')"  min-width="70" class-name="text-center" align="center">
                            <template slot-scope="scope">
                                <!-- {{scope.row.osVersion}} -->
                                <ul class="text-right inline-block ">
                                    <li class="inline-block mr5">
                                        <!-- 操作系统类型 -->
                                        <el-tooltip class="item" effect="light" :content="scope.row.osVersion || scope.row.osType" :disabled="!scope.row.osVersion && !scope.row.osType" placement="top">
                                            <zt-icon class="font14" :icon="scope.row.imagePic || getIcon(scope.row.osVersion)" type="svg"></zt-icon>
                                        </el-tooltip>
                                    </li>
                                    <!-- 点击查看：监控信息 -->
                                    <!-- <li class="inline-block">
                                        <el-tooltip placement="top" effect="light" transition="scale-in">
                                            <div slot="content">
                                                <a @click="changeCurrentEcsInst(scope.row)">{{ $t('ecs.inst.list.clickToView') }}</a>
                                            </div>
                                            <i class="iconfont icon-fuwuqi font16"></i>
                                        </el-tooltip>
                                    </li>                                                 -->
                                </ul>
                            </template>
                        ...
                        <!-- 网络类型 -->
                        <el-table-column prop="nettype" :label="getLabel('nettype')" min-width="70">
                            <template slot-scope="scope">
                                {{ scope.row.nettype | nettype}}
                            </template>
                        </el-table-column>
                        <!-- 配置  -->
                            ...
                            <template slot-scope="scope">
                                <ul class="left">
                                    ...
                                    <li v-if="!scope.row.toCpu" class="">
                                        <span class="text-nowrap">vCPU: {{scope.row.cpu}}{{ $t('common.cpuAlias') }}</span>
                                    </li>
                                    <li v-if="!scope.row.toCpu" class="">
                                        <span class="text-nowrap">{{$t('ecs.inst.list.memory')}}：{{scope.row.ram/1024 }}GiB</span>
                                    </li>
                                    ...
                                    <li v-if="!!scope.row.toCpu" class="">
                                        <span class="">{{$t('ecs.inst.list.configChanged')}}<br/> {{ scope.row.instanceStatus == 'CHANGE' ? $t('common.state.rebootingAlias') : $t('common.state.awaitReboot')}} </span>
                                    </li>
                                </ul>
                            </template>
                        </el-table-column>
                        <el-table-column prop="paytype" :label="getLabel('paytype')" min-width="70">
                            <template slot-scope="scope">
                                {{ (scope.row.payInfo && scope.row.payInfo.paytype) || $t('ecs.inst.list.freeTrial')}}
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('common.tableHeaderOperateText')" key="op" width="260" class-name="option-column text-nowrap">
                            <template slot-scope="scope">
                                <!-- 管理 -->
                                <router-link :disabled="dropdownActive(scope.row.instanceStatus, modifyConfigActivedStatus)" :to="{name:'app.ecs.inst.detail',params:{id:scope.row.id,item:scope.row}}">{{$t('ecs.inst.list.manage') }}</router-link>
                                <b class="link-division-symbol"></b>
                                <!-- 远程登录 -->
                                <a @click="getVncBefore(scope.row)" :disabled="dropdownActive(scope.row.instanceStatus, remoteLoginActivedStatus)">{{$t('ecs.inst.list.remoteLogin') }}</a>
                                <b class="link-division-symbol"></b>
                                <!-- 更改实例规格 -->
                                <router-link :to="{name:'app.ecs.changeconfig',params:{id:scope.row.id,item:scope.row}}" :disabled="dropdownActive(scope.row.instanceStatus, modifyConfigActivedStatus)">{{$t('ecs.inst.list.modifyConfig') }}</router-link>
                                <b class="link-division-symbol"></b>
                                <!-- 更多 -->
                                <zt-dropdown size="small" trigger="click"  :placement="getPlacement(scope.$index)">
                                    <span class="el-dropdown-link">
                                        {{$t('common.more')}}<i class="el-icon-caret-bottom el-icon--right"></i>
                                    </span>
                                    <zt-dropdown-menu slot="dropdown" :placement="getPlacement(scope.$index)">
                                        <zt-dropdown-item v-tooltip.left="dropdownActive(scope.row.instanceStatus, item.activedStatus) ? item.tip : ''" 
                                        v-for="item in ECS_DROPDOWN" :key="item.text" :disabled="!item.children && dropdownActive(scope.row.instanceStatus, item.activedStatus)">
                                            {{item.text}}
                                            <zt-dropdown-menu slot="dropdown" v-if="item.children"  >
                                                <zt-dropdown-item 
                                                @click="dropdownClickHandler(child, scope.row)"
                                                v-tooltip.left="dropdownActive(scope.row.instanceStatus, child.activedStatus) ? child.tip : ''" 
                                                v-for="child in getChildren(item,scope.row)" :key="child.text" 
                                                :disabled="dropdownActive(scope.row.instanceStatus, child.activedStatus)">                                               
                    ...
                    <zt-affix :offset-bottom="0" @change="onAffixChange" v-if="!loading" class="affix-pagination">
