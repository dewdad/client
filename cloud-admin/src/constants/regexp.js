 ...
 * @Last Modified time: 2018-08-27 19:36:40
 * 常量正则表达式配置文件
 */
 ...
 * 账号名 只能包含中英文，数字，下划线
 */
export const USERNAME_REGEXP = new RegExp(/^[[\u4e00-\u9fa5a-zA-Z0-9_]+$/);
 ...
 * 创建实例登录密码
 */
export const INST_PASSWORD = /^(?=^.{8,30}$)((?=.*\\d)(?=.*[A-Z])(?=.*[a-z])|(?=.*\\d)(?=.*[^A-Za-z0-9])(?=.*[a-z])|(?=.*[^A-Za-z0-9])(?=.*[A-Z])(?=.*[a-z])|(?=.*\\d)(?=.*[A-Z])(?=.*[^A-Za-z0-9]))^.*$/;
